...
# -- SELECT COUNT(*) FROM vibepay_portal.vibepay_payin_report where `Date Time` LIKE '2025-07-07%';

# SELECT COUNT(*) FROM vibepay_portal.vibepay_payin_wallet where `Date` LIKE '2025-07-07%';

# SELECT COUNT(*) FROM vibepay_portal.vibepay_payout_report where `Date Time` LIKE '2025-07-07%';

# SELECT COUNT(*) FROM vibepay_portal.vibepay_payout_wallet where `Date` LIKE '2025-07-07%';

# SELECT COUNT(*) FROM vibepay_portal.vibepayin_add_fund_report where `Date` LIKE '2025-07-07%';


# SELECT COUNT(*) FROM sabpaisa.payin where `Transaction Date` LIKE '2025-07-07%';

# SELECT COUNT(*) FROM sabpaisa.settelment_report where `TRANSACTION DATE` LIKE '2025-07-07%';



# SELECT count(*) FROM payonetic_portal.payonetic_payout_report where `Date Time` LIKE '2025-07-07%';

# SELECT count(*) FROM payonetic_portal.payonetic_payout_wallet where `Date Time` LIKE '2025-07-07%';

# SELECT count(*) FROM payonetic_portal.payonetic_add_fund_report where `Date Time` LIKE '2025-07-07%';

# SELECT count(*) FROM swiftsend_portal.swiftsend_payout_report where `Date Time` LIKE '2025-07-07%';

# SELECT count(*) FROM swiftsend_portal.swiftsend_add_fund_report where `Date` LIKE '2025-07-07%';

# SELECT count(*) FROM swiftsend_portal.swiftsend_payout_wallet where `Date` LIKE '2025-07-07%';
...
class ConfigManager:
    ...
    def get_table_instructions(cls, db_name: str, table_name: str) -> str:
        ...
        instructions = {
            ...
            # New instructions for the added databases/tables
...
class DataProcessor:
    ...
    def format_dates(df: pd.DataFrame, logger) -> pd.DataFrame:
        ...
        """
        date_related_keywords = ['date', 'time', 'datetime', 'timestamp', 'created', 'modified']
...
class DateTimeConverter:
    ...
    def _identify_date_columns(df: pd.DataFrame) -> List[str]:
        ...
        date_cols = []
        date_keywords = ['date', 'time', 'timestamp', 'created', 'modified', 'at']
...
class MySQLExcelUploaderGUI:
    ...
    def setup_ui(self):
        ...
        # Ensure the theme supports custom styling
        style.theme_use('default')

        # Create a custom style for the upload button
        style.configure(
            "Upload.TButton",
        ...
        # Connection Frame
        connection_frame = ttk.LabelFrame(main_frame, text="Database Connection", padding="10")
        connection_frame.pack(fill='x', pady=(0, 10))

        # Connection Fields
        ttk.Label(connection_frame, text="Host:").grid(row=0, column=0, sticky='w', padx=5, pady=5)
        self.host_entry = ttk.Entry(connection_frame, show="-")
        self.host_entry.grid(row=0, column=1, sticky='ew', padx=5, pady=5)
        self.host_entry.insert(0, "")

        ttk.Label(connection_frame, text="Username:").grid(row=1, column=0, sticky='w', padx=5, pady=5)
        self.username_entry = ttk.Entry(connection_frame, show="%")
        self.username_entry.grid(row=1, column=1, sticky='ew', padx=5, pady=5)
        self.username_entry.insert(0, "")

        ttk.Label(connection_frame, text="Password:").grid(row=2, column=0, sticky='w', padx=5, pady=5)
        self.password_entry = ttk.Entry(connection_frame, show="*")
        self.password_entry.insert(0, "")
        self.password_entry.grid(row=2, column=1, sticky='ew', padx=5, pady=5)

        # Database Selection
        ttk.Label(connection_frame, text="Database.Table:").grid(row=3, column=0, sticky='w', padx=5, pady=5)
        self.db_table_var = tk.StringVar(value=ConfigManager.get_database_options()[0])
        ...
        db_table_dropdown.grid(row=3, column=1, sticky='ew', padx=5, pady=5)
        ...
        upload_btn = ttk.Button(
            ...
            style="Upload.TButton"  # use your custom style name here
        )
        upload_btn.grid(row=1, column=0, padx=5, pady=5, sticky='ew')


        # Generate PDF Button (side by side)
        generate_pdf_btn = ttk.Button(
            upload_frame,
            text="Generate PDF Report",
            command=self.generate_pdf_button_handler
        )
        generate_pdf_btn.grid(row=1, column=1, padx=5, pady=5, sticky='ew')
    ...
    def generate_pdf_report(self, database, table, file_list, total_rows, log_lines):
        """Save a minimal PDF report with only essential info"""
        try:
            ...
            # Filter only the 4 summary lines
            filtered_lines = []
            for line in log_lines:
                if (
                    "FILES COMBINED AND UPLOADED SUCCESSFULLY" in line
                    or "Target Database:" in line
                    or "Target Table:" in line
                    or "Total rows uploaded:" in line
                ):
            ...
            c.drawString(50, y, "UPLOAD SUMMARY REPORT")
            y -= 40
